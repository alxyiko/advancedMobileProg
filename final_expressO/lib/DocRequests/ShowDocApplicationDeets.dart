...
class ShowApplicationDeets extends StatefulWidget {
  const ShowApplicationDeets({super.key});

  @override
  State<ShowApplicationDeets> createState() => _ShoRrequirementsStateList();
}

class _ShoRrequirementsStateList extends State<ShowApplicationDeets> {
  final GlobalKey<FormState> _requestForm = GlobalKey<FormState>();
  bool success = true;
  bool loading = true;
  String remarkText = selectedApplication?['status'] == 'denied' ? "due to ${selectedApplication?['remarks']}" : "";
  String timeText = selectedApplication?['updatedAt'] != null
      ? 'on ${Firebaseuserservice.formatTimestamp(selectedApplication?['updatedAt'])}'
      : "";
  String requestTitle = selectedApplication?['title'] as String;
  String? file;
  ...
  void initState() {
    super.initState();
    print('requestedDoc');
    requirements = (jsonDecode(selectedApplication?['requirmentFiles']))
            ?.map((req) => {
                  'reqName': req['reqName'],
                  'file': req['file'],
                }) // Use null initially
    ...
    // print(jsonDecode(selectedApplication?['requirmentFiles']));
    getFirestoreEntry(selectedApplication?['programApplied']);
  }

  Future<void> getFirestoreEntry(DocumentReference docRef) async {
    try {
      ...
      if (doc.exists) {
        setState(() {
          loading = false;
          program = doc.data() as Map<String, dynamic>;
        });
      } else {
                    ...
                    child: Column(
                      children: [
                        ...
                        Align(
                            alignment: Alignment.centerLeft,
                            child: Text(
                              'Status: ${selectedApplication?['status'] != null ? selectedApplication!['status'] : 'Pending'} ',
                              style: const TextStyle(
                                color: Color(0xFF5E5E5E),
                                fontSize: 20,
                                fontFamily: 'Rubik',
                        ...
                        Align(
                            ...
                            child: loading
                                ...
                                : Text(
                                    'This program will occur at ${Firebaseuserservice.formatTimestamp(program?['eventDate'])} at ${program?['venue']}, be there!',
                                    style: const TextStyle(
                                      color: Color(0xFF5E5E5E),
                                      fontSize: 14,
                                      fontFamily: 'Rubik',
                ...
                Column(
                  children: [
                    SizedBox(
                        ...
                        child: selectedApplication?['status'] == null ||
                            ...
                            : Text(
                                'This request has been ${selectedApplication?['status']} $timeText $remarkText')),
