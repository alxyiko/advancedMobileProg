...
Widget buildListView(List<Map<String, dynamic>> items,
  ...
  return ListView.builder(
    ...
    itemBuilder: (context, index) {
      bool isSelected = requestedDoc?['name'] == items[index]['name'];

      return Container(
        margin: const EdgeInsets.symmetric(vertical: 5), // Margin outside
        decoration: BoxDecoration(
          color:
              isSelected ? const Color(0xFF006644) : Colors.white, // Background color
          borderRadius: BorderRadius.circular(5), // Rounded corners
          border: Border.all(
            color: isSelected
                ? const Color(0xFF004D33)
                : const Color(0xFFC3C3C3), // Border color
...
//     itemBuilder: (context, index) {
//       return ListTile(
//         tileColor: requestedDoc?['name'] == items[index]['name'] ? Color(0xFF006644) :Colors.white,
//         textColor: requestedDoc?['name'] != items[index]['name'] ? Color(0xFF006644) :Colors.white,
...
class ShowDoclist extends StatefulWidget {
  const ShowDoclist({super.key});

  @override
  State<ShowDoclist> createState() => _ShowDoclistState();
}

class _ShowDoclistState extends State<ShowDoclist> {
  ...
  Future<void> fetchDocList() async {
    final data = await Firebaseuserservice.getAvailableItems('documents');
    print('excepmtedList');
    print(excepmtedList);
  ...
  void handleNext(BuildContext context) {
    if (requestedDoc == null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a document before proceeding.'),
                        ...
                        child: Column(children: [
                          ...
                          SizedBox(
                            ...
                            child: FloatingActionButton.extended(
                              ...
                              backgroundColor:
                                  const Color(0xFF006644), // Custom color
