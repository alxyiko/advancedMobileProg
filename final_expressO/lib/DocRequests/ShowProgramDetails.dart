      ...
      return GestureDetector(
        ...
        child: Card(
          ...
          child: Column(
            ...
            children: [
              Stack(
                alignment: Alignment.center, // Centers the button on the image/container
                children: [
                  ClipRRect(
                    borderRadius: const BorderRadius.only(
                      topLeft: Radius.circular(12), // Round only the top left
                      topRight: Radius.circular(12), // Round only the top right
                    ),
                    ...
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor: items[index]['file'] != null
                            ? Colors.black.withOpacity(0.5) // Slightly black with low opacity
                            : Colors.grey.withOpacity(0), // Light gray for no picture
                      ...
                      child: Column(
                        mainAxisSize: MainAxisSize.min, // Keeps button height minimal
                        children: [
                          Icon(
                            Icons.camera_alt,
                            color: items[index]['file'] != null ? Colors.white : Colors.grey, // White when there's a picture, gray when none
                            size: 40,
                          ),
                          const SizedBox(height: 4), // Space between icon and text
                          Text(
                            items[index]['file'] != null ? 'Retake Photo' : 'Take Photo',
                            style: TextStyle(
                              color: items[index]['file'] != null ? Colors.white : Colors.grey,
                              fontSize: 16,
                            ),
                          ),
                        ],
...
class ShowProgramRequirementsList extends StatefulWidget {
  const ShowProgramRequirementsList({super.key});

  @override
  State<ShowProgramRequirementsList> createState() => _ShoRrequirementsStateList();
}

class _ShoRrequirementsStateList extends State<ShowProgramRequirementsList> {
  final GlobalKey<FormState> _requestForm = GlobalKey<FormState>();
  bool success = true;
  bool loading = false;
  

  String progName = appliedProgram?['name'] as String;
  XFile? file;
  ...
  void initState() {
    ...
    requirements = (appliedProgram?['requirements'] as List<dynamic>?)
            ?.map((req) =>
  ...
  Future<void> submitRequirementPhotos() async {
    ...
    DocumentReference docRef = FirebaseFirestore.instance
        .collection('/barangayNexus/documents/programs')
        .doc(appliedProgram?['id']);

    await Firebaseuserservice.addApplication({
      'title':
          "Application for ${appliedProgram?['name']}",
      'entityID': appliedProgram?['id'],
      'programApplied': docRef,
      'applicant': uid,
    ...
    //   'title':
    //       "$_copies ${_copies! > 1 ? 'copies' : 'copy'} of ${appliedProgram?['name']}",
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Application Successfully submitted!'),
            ...
            content: Column(
              ...
              SizedBox(
                ...
                Align(
                  ...
                  child: Text("This will only take a moment while\nwe process your request"),),  // Loading message
              ],
            ),
          );
        },
  ...
  void backToDocList() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('You need to select a program first!')),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: !appliedProgram!.containsKey('requirements')
            ? const Center(
                  ...
                  child: Column(
                    ...
                    children: [
                      Form(
                            ...
                            children: [
                              ...
                              const Align(
                               ...
                               Align(
                                alignment: Alignment.centerLeft,
                                child: Text(
                                  '${appliedProgram?['desc']} \n\nWhen: ${ Firebaseuserservice.formatTimestamp(appliedProgram?['eventDate'])} at ${appliedProgram?['venue']} ',
                                  style: const TextStyle(
                                    color: Color(0xFF2E2E2E),
                                    fontSize: 16,
                                    fontWeight: FontWeight.bold,
                          ...
                          //       const SizedBox(height: 5), // Adds space between the text and the container
                      ...
                      Column(
                        children: [
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              style: ElevatedButton.styleFrom(
                                padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 18), // Custom padding
                              ),
                              onPressed: () async {
                                await _submitForm();
                              },
                              child: const Text('Finalize', style: TextStyle(color: Colors.white, fontSize: 16)),
                            ),
                          ),
