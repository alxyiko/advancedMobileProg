    ...
    itemBuilder: (context, index) {
      return Container(
        ...
        child: ListTile(
          ...
          title: Text(
            '${items[index]["action"]} on  ${Firebaseuserservice.formatTimestamp(items[index]["createdAt"] as Timestamp)}',
            // Assuming this is your item text
...
class _ShowHistoryState extends State<ShowHistory> {
  ...
  bool searched = false;
  final notifUid = FirebaseAuth.instance.currentUser?.uid;
  ...
  Future<void> fetchDocsRequests() async {
    final data = await Firebaseuserservice.getList(notifUid as String, 'history');
